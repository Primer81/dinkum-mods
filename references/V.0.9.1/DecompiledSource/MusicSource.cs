...
public class MusicSource
{
	...
	public void adjustLocalVolume(float difference)
	{
		...
		myAudioSource.volume = Mathf.Clamp(localVolume, 0f, fadeMax) * (MusicManager.manage.musicMasterVolume * SoundManager.Instance.getMasterVolume() / 4f);
	}
	...
	public void checkForLastSecondsFade()
	{
		...
		else if (myAudioSource.time < 20f)
		{
	...
	{
		myAudioSource.time = 82f;
	}

	public bool isCurrentClipPlaying(AudioClip compare)
	{
		if (myAudioSource.clip == compare)
		{
			return true;
		}
		return false;
