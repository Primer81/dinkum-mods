...
public class VehicleMakeParticles : MonoBehaviour
{
	...
	public bool hasVerticleDirectionAnimation;
	...
	private Vector3 oldVertPosition;
	...
	public ASound boostSound;

	public AudioSource boostOnSound;

	public float boostVolume;

	private bool showingBoost;
	...
	private void FixedUpdate()
	{
		...
		if (hasVerticleDirectionAnimation)
		{
			vertDir = Mathf.Lerp(vertDir, upOrDown(oldVertPosition, base.transform.position), Time.deltaTime * turnSpeedAnimSpeed / 2f);
			oldVertPosition = base.transform.position;
			myAnim.SetFloat("VerticalDir", vertDir);
		}
		if (useGroundedSounds)
		{
			if (lastGrounded != myControl.isGrounded())
			{
				...
				{
					SoundManager.Instance.playASoundAtPoint(onGroundedSound, base.transform.position);
				...
				{
					SoundManager.Instance.playASoundAtPoint(onNotGroundedSound, base.transform.position);
					if (splashWhenGrouned)
					{
						Transform[] array = particlePositions;
		...
		if ((bool)dragSound)
		{
			...
			{
				dragSound.volume = Mathf.Lerp(dragSound.volume, movementSpeed / myControl.maxSpeed * dragVolumeMax * SoundManager.Instance.GetGlobalSoundVolume(), Time.deltaTime * 10f);
			}
			else
			{
		...
		if ((bool)dragWhenGrounded)
		{
			if (myControl.isGrounded())
			{
				dragWhenGrounded.volume = Mathf.Lerp(dragWhenGrounded.volume, movementSpeed / myControl.maxSpeed * dragGroundedMax * SoundManager.Instance.GetGlobalSoundVolume(), Time.deltaTime * 10f);
			}
			else
			{
	...
	private void Update()
	{
		...
		if (myControl.hasBoost)
		{
			if (showingBoost != myControl.currentlyBoosting)
			{
				showingBoost = myControl.currentlyBoosting;
				if (myControl.currentlyBoosting)
				{
					SoundManager.Instance.playASoundAtPoint(boostSound, base.transform.position);
				}
			}
			if (myControl.currentlyBoosting)
			{
				for (int i = 0; i < boostParts.Length; i++)
				{
					boostParts[i].Emit(1);
				}
				boostVolume = Mathf.Clamp(boostVolume + Time.deltaTime / 2f, 0f, engineMaxVolume * 2f);
				boostOnSound.volume = boostVolume * SoundManager.Instance.GetGlobalSoundVolume();
				boostOnSound.pitch = Mathf.Clamp(boostOnSound.pitch + Time.deltaTime * 2f, 1f, 6f);
			}
			else
			{
				boostVolume = Mathf.Clamp(boostVolume - Time.deltaTime / 2f, 0f, engineMaxVolume * 2f);
		...
		if (animateGrounded)
		{
			...
			{
				myAnim.SetBool("Grounded", value: true);
			}
			else
			{
				myAnim.SetBool("Grounded", value: false);
			}
	...
	public void makeMoveParticles()
	{
		...
		foreach (Transform transform in array)
		{
			ParticleManager.manage.emitFeetDust(WorldManager.Instance.tileTypes[WorldManager.Instance.tileTypeMap[(int)base.transform.position.x / 2, (int)base.transform.position.z / 2]].footStepParticle, transform.position, transform.rotation);
		}
	}
	...
	public float upOrDown(Vector3 oldPos, Vector3 currentPos)
	{
		if (currentPos.y > oldPos.y + 0.04f)
		{
			return 1f;
		}
		if (currentPos.y < oldPos.y - 0.04f)
	...
	private IEnumerator makeVehicleSounds()
	{
		...
		float gearTimer = 0f;
		_ = (int)(num * numberOfGears);
		while (true)
		{
			if (myControl.canFly && myVehicle.hasDriver())
			{
				animationSpeedGoal = Mathf.Lerp(animationSpeed, 2f, Time.deltaTime * 2f);
				myAudio.volume = Mathf.Lerp(myAudio.volume, engineMaxVolume * SoundManager.Instance.GetGlobalSoundVolume(), Time.deltaTime);
				if (myControl.isGrounded())
				{
	...
	private IEnumerator movementParticles()
	{
		...
		while (true)
		{
			...
			for (int i = 0; i < particlePositions.Length; i++)
			{
				...
				{
					if (!WorldManager.Instance.isPositionOnMap((int)particlePositions[i].position.x / 2, (int)particlePositions[i].position.z / 2) || (WorldManager.Instance.waterMap[(int)particlePositions[i].position.x / 2, (int)particlePositions[i].position.z / 2] && particlePositions[i].position.y <= 0.6f))
					{
						ParticleManager.manage.waterSplash(particlePositions[i].position, 4);
					}
					else
					{
						ParticleManager.manage.emitFeetDust(WorldManager.Instance.tileTypes[WorldManager.Instance.tileTypeMap[(int)particlePositions[i].position.x / 2, (int)particlePositions[i].position.z / 2]].footStepParticle, particlePositions[i].position, particlePositions[i].rotation);
					}
				}
				if (!WorldManager.Instance.isPositionOnMap((int)particlePositions[i].position.x / 2, (int)particlePositions[i].position.z / 2) || (WorldManager.Instance.waterMap[(int)particlePositions[i].position.x / 2, (int)particlePositions[i].position.z / 2] && particlePositions[i].position.y <= 0.6f))
				{
