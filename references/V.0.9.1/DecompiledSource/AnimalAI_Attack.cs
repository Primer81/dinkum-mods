...
public class AnimalAI_Attack : NetworkBehaviour
{
	...
	public bool chasingObjectHigherOrLower()
	{
		...
		{
			int num = WorldManager.Instance.heightMap[Mathf.RoundToInt(currentlyAttacking.position.x / 2f), Mathf.RoundToInt(currentlyAttacking.position.z / 2f)] - WorldManager.Instance.heightMap[Mathf.RoundToInt(base.transform.position.x / 2f), Mathf.RoundToInt(base.transform.position.z / 2f)];
			if (num == 2)
			{
				return true;
			}
	...
	private void RpcBasicAttack()
	{
		...
		SendRPCInternal(typeof(AnimalAI_Attack), "RpcBasicAttack", writer, 0, includeOwner: true);
	...
	{
		PooledNetworkWriter writer = NetworkWriterPool.GetWriter();
		SendRPCInternal(typeof(AnimalAI_Attack), "RpcPlayWindUp", writer, 0, includeOwner: true);
		NetworkWriterPool.Recycle(writer);
	}
	...
	{
		PooledNetworkWriter writer = NetworkWriterPool.GetWriter();
		SendRPCInternal(typeof(AnimalAI_Attack), "RpcPlayWindUpAlt", writer, 0, includeOwner: true);
		NetworkWriterPool.Recycle(writer);
	}
	...
	private void RpcPlayTiredParts(float timer)
	{
		...
		SendRPCInternal(typeof(AnimalAI_Attack), "RpcPlayTiredParts", writer, 0, includeOwner: true);
		NetworkWriterPool.Recycle(writer);
	}
	...
	{
		PooledNetworkWriter writer = NetworkWriterPool.GetWriter();
		SendRPCInternal(typeof(AnimalAI_Attack), "RpcCancelAttack", writer, 0, includeOwner: true);
		NetworkWriterPool.Recycle(writer);
	}
	...
	{
		setCurrentlyPlayingAttack(newPlayingAttack: false);
	}
	...
	public void setNewCurrentlyAttacking(Transform newAttacking)
	{
		...
		if (currentlyAttacking != null)
		{
			...
			else
			{
				...
				if ((bool)currentlyAttacking)
				{
					...
					if (((bool)component && component.isCurrentlyTalking) || ((bool)component && component.myPickUp.sittingPos != Vector3.zero) || ((bool)component && !NetworkMapSharer.Instance.nextDayIsReady))
					{
						currentlyAttacking = null;
						currentlyAttacking_Damageable = null;
					}
		...
		if (num && currentlyAttacking != null && myAi.myAgent.isActiveAndEnabled)
		{
	...
	public void removeAttackingChar()
	{
		...
		{
			currentlyAttackingChar.RemoveAnEnemy(this);
			currentlyAttackingChar = null;
		}
	}
	...
	public void setNewAttackingChar()
	{
		...
		if ((bool)currentlyAttacking_Damageable && (bool)currentlyAttacking_Damageable.myChar)
		{
			if ((bool)currentlyAttackingChar && currentlyAttacking_Damageable.myChar != currentlyAttackingChar)
			{
				...
				currentlyAttackingChar.AddAnEnemy(this);
			}
	...
	private IEnumerator chaseAndThenJumpUpOrDown()
	{
		...
		posToCheckHeight.y = WorldManager.Instance.heightMap[Mathf.RoundToInt(posToCheckHeight.x / 2f), Mathf.RoundToInt(posToCheckHeight.z / 2f)];
	...
	{
		setCurrentlyPlayingAttack(newPlayingAttack: true);
		if ((bool)myAnim)
		{
