...
public class CharacterHeadIK : MonoBehaviour
{
	...
	private void Update()
	{
		...
		else if (myEquip.isInVehicle())
		{
			bodyWeight = 0f;
			lookingAtWeight = 0f;
			eyes.eyeLookAtTrans.localPosition = Vector3.zero;
		}
		else if (myEquip.checkIfDoingEmote() || isEating || myEquip.IsDriving() || myPickup.isLayingDown() || myDamage.health <= 0)
		{
			bodyWeight = Mathf.Lerp(bodyWeight, 0f, Time.deltaTime * 15f);
			lookingAtWeight = Mathf.Lerp(lookingAtWeight, 0f, Time.deltaTime * 8f);
			eyes.eyeLookAtTrans.localPosition = Vector3.zero;
		}
		else if ((myEquip.usingItem && (bool)myEquip.itemCurrentlyHolding && myEquip.itemCurrentlyHolding.useRightHandAnim) || (myEquip.lookLock && (bool)myEquip.itemCurrentlyHolding && myEquip.itemCurrentlyHolding.useRightHandAnim))
		{
			bodyWeight = Mathf.Lerp(bodyWeight, 0f, Time.deltaTime * 15f);
			lookingAtWeight = Mathf.Lerp(lookingAtWeight, 0f, Time.deltaTime * 8f);
	...
	public void moveLookTrans(Vector3 desiredPos, float multiplier)
	{
		...
		_ = lookTrans.position - base.transform.position;
