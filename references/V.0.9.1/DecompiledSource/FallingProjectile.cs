...
public class FallingProjectile : MonoBehaviour
{
	...
	private IEnumerator flyUpAndHitGround()
	{
		...
		skyObject.SetActive(value: false);
		SoundManager.Instance.playASoundAtPoint(smashSound, base.transform.position);
		hitGroundObject.SetActive(value: true);
	...
	public void checkForDamage()
	{
		...
		if (hitGround && WorldManager.Instance.isPositionOnMap(base.transform.position) && WorldManager.Instance.waterMap[Mathf.RoundToInt(base.transform.position.x / 2f), Mathf.RoundToInt(base.transform.position.z / 2f)])
		{
			stayOnGroundTime = 0f;
		}
		...
		else if (!hitGround && WorldManager.Instance.isPositionOnMap(base.transform.position) && base.transform.position.y <= (float)WorldManager.Instance.heightMap[Mathf.RoundToInt(base.transform.position.x / 2f), Mathf.RoundToInt(base.transform.position.z / 2f)])
		{
			hitGround = true;
		}
	}
	...
	public void DoFakeShadow()
	{
		if ((bool)fakeShadow && WorldManager.Instance.isPositionOnMap(base.transform.position))
		{
			fakeShadow.transform.position = new Vector3(fakeShadow.transform.position.x, (float)WorldManager.Instance.heightMap[Mathf.RoundToInt(fakeShadow.transform.position.x / 2f), Mathf.RoundToInt(fakeShadow.transform.position.z / 2f)] + 0.1f, fakeShadow.transform.position.z);
		}
	}
}
