	...
	private IEnumerator soundCheck()
	{
		while (true)
		{
			if (myChar.myEquip.usingItem)
			{
				...
				useSource.volume = Mathf.Lerp(useSource.volume, useVolume * SoundManager.Instance.GetGlobalSoundVolume(), Time.deltaTime * 25f);
			}
			else
			{
				idleSource.volume = Mathf.Lerp(idleSource.volume, idleVolume * SoundManager.Instance.GetGlobalSoundVolume(), Time.deltaTime * 25f);
