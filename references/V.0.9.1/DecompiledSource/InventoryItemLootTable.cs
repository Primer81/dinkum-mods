...
public class InventoryItemLootTable : MonoBehaviour
{
	...
	public InventoryItem getRandomDropFromTable(MapRand generator = null)
	{
		...
		{
			Debug.LogError(base.gameObject?.ToString() + " Loot table rarity does not match item length - Was created at runtime to work. Check this out James");
		...
		float num2 = generator?.Range(0f, num) ?? Random.Range(0f, num);
		float num3 = 0f;
