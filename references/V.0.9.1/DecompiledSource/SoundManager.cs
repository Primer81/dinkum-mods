...
public class SoundManager : MonoBehaviour
{
	...
	public ASound carpetFootStep;

	public ASound steelFootStep;

	public ASound plasticFootStep;
	...
	public ASound cameraSound;

	public ASound underwaterFootSteps;

	public ASound carHorn;

	public ASound extinguishFire;

	public ASound clearFoodAndHunger;

	public ASound wishMadeSound;

	public ASound extinguishLavaSound;

	public ASound bigStoneGrinderSound;

	public ASound slowDownTime;

	public ASound speedUpTime;

	public ASound transformSound;

	public ASound changeWallOrFloorSound;

	public ASound townBellSound;

	private bool sayingAWord;

	private bool underWater;

	private bool canPlayStunnedSound = true;
	...
	{
		onMasterChange.RemoveAllListeners();
		Instance = null;
	}
	...
	public void playASoundAtPoint(ASound soundToPlay, Vector3 position, float volumePercent = 1f, float pitchDif = 1f)
	{
		...
		foreach (AudioSource audioSource in array)
		{
			if (!audioSource.isPlaying)
			{
				...
				audioSource.PlayOneShot(soundToPlay.getSound(), (soundToPlay.volume + Random.Range((0f - soundToPlay.volume) / 10f, soundToPlay.volume / 10f)) * volumePercent * GetGlobalSoundVolume());
				break;
			}
		}
	}
	...
	public float playASoundAtPointAndReturnLength(ASound soundToPlay, Vector3 position, out AudioSource currentlyPlaying, float volumePercent = 1f, float pitchDif = 1f)
	{
		...
		foreach (AudioSource audioSource in array)
		{
			if (!audioSource.isPlaying)
			{
				...
				audioSource.PlayOneShot(sound, (soundToPlay.volume + Random.Range((0f - soundToPlay.volume) / 10f, soundToPlay.volume / 10f)) * volumePercent * GetGlobalSoundVolume());
	...
	public void playASoundAtPointWithPitch(ASound soundToPlay, Vector3 position, float pitch)
	{
		...
		foreach (AudioSource audioSource in array)
		{
			if (!audioSource.isPlaying)
			{
				...
				audioSource.PlayOneShot(soundToPlay.getSound(), soundToPlay.volume * GetGlobalSoundVolume());
				break;
			}
		}
	}
	...
	public bool canPlayStunnedByLightSound()
	{
		return canPlayStunnedSound;
	}

	public void playStunnedByLightSound()
	{
		canPlayStunnedSound = false;
		StartCoroutine(countDownStunByLightSound());
	}

	private IEnumerator countDownStunByLightSound()
	{
		yield return new WaitForSeconds(0.1f);
		canPlayStunnedSound = base.transform;
	}
	...
	{
		yield return null;
		myAudios[audio].PlayOneShot(soundToPlay.getSound(), soundToPlay.volume * GetGlobalSoundVolume());
