...
public class AnimalAI_Sleep : NetworkBehaviour
{
	...
	private void Start()
	{
		...
		isRideable = GetComponent<Vehicle>();
	}
	...
	private bool sleepDesiresMet()
	{
		...
		if ((bool)isRideable && isRideable.hasDriver())
		{
			return false;
		}
	...
	public IEnumerator checkIfNeedsSleep()
	{
		...
		{
			if (!isSleeping && (!isPet || isPet.followingId == 0) && (!isRideable || !isRideable.hasDriver()))
			{
				if ((bool)mySleepSpot)
				{
				...
				else
				{
					...
					if (!mySleepSpot && !WorldManager.Instance.waterMap[Mathf.RoundToInt(base.transform.position.x / 2f), Mathf.RoundToInt(base.transform.position.z / 2f)] && Random.Range(0, 15) == 8)
					{
		...
		else if (!isSleeping && sleepDesiresMet() && myAi.myAgent.isActiveAndEnabled)
		{
			...
			else
			{
				...
				normalized3 = new Vector3(normalized3.x, WorldManager.Instance.heightMap[(int)normalized3.x / 2, (int)normalized3.z / 2], normalized3.z);
