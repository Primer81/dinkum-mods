...
public class TownManagerSave
{
	...
	public void saveTown()
	{
		...
		savedPos[0] = NetworkMapSharer.Instance.localChar.transform.position.x;
		savedPos[1] = NetworkMapSharer.Instance.localChar.transform.position.y;
		savedPos[2] = NetworkMapSharer.Instance.localChar.transform.position.z;
		savedInside[0] = TownManager.manage.savedInside[0];
		savedInside[1] = TownManager.manage.savedInside[1];
		savedRotation[0] = NetworkMapSharer.Instance.localChar.transform.eulerAngles.y;
		...
		mineLevelSaved = RealWorldTimeLight.time.mineLevel;
	}
	...
	public void load()
	{
		...
		if (savedPos != null)
		{
			...
			WorldManager.Instance.spawnPos.position = TownManager.manage.lastSleptPos;
		}
		...
		{
			NetworkMapSharer.Instance.NetworkprivateTowerPos = new Vector2(privateTowerPos[0], privateTowerPos[1]);
		}
		...
		RealWorldTimeLight.time.NetworkmineLevel = mineLevelSaved;
	}
}
