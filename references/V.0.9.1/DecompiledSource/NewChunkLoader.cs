...
public class NewChunkLoader : MonoBehaviour
{
	...
	{
		chunkViewDistance = Mathf.Clamp(newDistance, 3, 6);
		...
		while (true)
		{
			if (!inside)
			{
				...
				if (num != centreChunkX || num2 != centreChunkY)
				{
					centreChunkX = num;
					centreChunkY = num2;
					WorldManager.Instance.returnChunksNotCloseEnough(centreChunkX, centreChunkY, chunkViewDistance);
					for (int y = -chunkViewDistance; y < chunkViewDistance; y++)
					{
						for (int x = -chunkViewDistance; x < chunkViewDistance; x++)
						{
							...
							if (WorldManager.Instance.doesPositionNeedsChunk(num3, num4))
							{
								WorldManager.Instance.getFreeChunkAndSetInPos(num3, num4);
								yield return null;
							}
						}
					...
					WorldManager.Instance.getNoOfWaterTilesClose((int)(Mathf.Round(viewingTransform.parent.position.x) / 2f) / chunkSize * chunkSize, (int)(Mathf.Round(viewingTransform.parent.position.z) / 2f) / chunkSize * chunkSize);
				}
			}
			yield return null;
		}
