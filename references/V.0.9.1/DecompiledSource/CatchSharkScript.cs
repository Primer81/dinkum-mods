...
public class CatchSharkScript : NetworkBehaviour
{
		...
		while (true)
		{
			...
			if (!myAttack.currentlyAttacking && !myAi.isInjuredAndNeedsToRunAway() && Physics.CheckSphere(position, 3.5f, lureLayer))
			{
				...
				if (array.Length != 0 && (bool)array[0].GetComponentInParent<BobberScript>() && array[0].GetComponentInParent<BobberScript>().catchSharks)
				{
					...
					while (lureInterestedIn.gameObject.activeInHierarchy && !isOnLand && fishingRodInterestedIn.fishOnLine == -1)
					{
						myAi.myAgent.SetDestination(lureInterestedIn.position);
						GetComponent<AnimateAnimalAI>().makeFishInterested(newInterested: true);
						yield return null;
						if (myAi.myAgent.stoppingDistance <= Vector3.Distance(myAi.myAgent.transform.position, lureInterestedIn.transform.position))
						{
							EquipItemToChar charEquip = fishingRodInterestedIn.GetComponent<EquipItemToChar>();
							int charHoldingId = -1;
							if ((bool)charEquip && fishingRodInterestedIn.fishOnLine == -1)
							{
								charHoldingId = charEquip.currentlyHoldingItemId;
								RpcBiteHook();
							}
							yield return new WaitForSeconds(1f);
							if (!charEquip || charHoldingId == -1 || charHoldingId != charEquip.currentlyHoldingItemId || fishingRodInterestedIn.fishOnLine != -1 || !fishingRodInterestedIn.isCastCompleted())
							{
								break;
							}
	...
	public void clearInterestInLure()
	{
		lureInterestedIn = null;
		fishingRodInterestedIn = null;
		GetComponent<AnimateAnimalAI>().makeFishInterested(newInterested: false);
	}
	...
	public void RpcBiteHook()
	{
		...
		SendRPCInternal(typeof(CatchSharkScript), "RpcBiteHook", writer, 0, includeOwner: true);
	...
	{
		if (WorldManager.Instance.heightMap[Mathf.RoundToInt(base.transform.position.x / 2f), Mathf.RoundToInt(base.transform.position.z / 2f)] > -1 || !WorldManager.Instance.waterMap[Mathf.RoundToInt(base.transform.position.x / 2f), Mathf.RoundToInt(base.transform.position.z / 2f)])
		{
	...
	private IEnumerator returnToWaterWait()
	{
		while (isOnLand)
		{
			...
			{
				Vector3 destination = WorldManager.Instance.findClosestWaterTile(base.transform.position, 10, checkIfFencedOff: true, -1);
	...
	{
		SoundManager.Instance.playASoundAtPoint(sharkJumpSound, base.transform.position);
	}
