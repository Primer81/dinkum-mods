...
public class MuseumDisplay : MonoBehaviour
{
	...
	public void updateExhibits()
	{
		...
		int num = (fishOnDisplay.Length + bugsOnDisplay.Length + underWaterCreaturesOnDisplay.Length) / 2;
		if (fishDonated + bugsDonated + crittersDonated >= 10)
		{
			shopStall1.gameObject.SetActive(value: true);
		}
		else
		{
			shopStall1.gameObject.SetActive(value: false);
		}
		if (fishDonated + bugsDonated + crittersDonated >= num)
		{
			shopStall2.gameObject.SetActive(value: true);
		}
		else
		{
			shopStall2.gameObject.SetActive(value: false);
		}
	...
	private void upDateUnderWaterCreatures()
	{
		...
		{
			if (MuseumManager.manage.underWaterCreaturesDonated[i])
			{
				if (MuseumManager.manage.underWaterCreaturesDonated[i])
				{
					crittersDonated++;
				}
				if (MuseumManager.manage.underWaterCreaturesDonated[i] && underWaterCreaturesOnDisplay[i] == null)
				{
	...
	{
		if (MuseumManager.manage.allFish[pos].fish.mySeason.myWaterLocation[0] == SeasonAndTime.waterLocation.NorthOcean || MuseumManager.manage.allFish[pos].fish.mySeason.myWaterLocation[0] == SeasonAndTime.waterLocation.SouthOcean || MuseumManager.manage.allFish[pos].fish.mySeason.myWaterLocation[0] == SeasonAndTime.waterLocation.ReefIsland)
		{
			fishOnDisplay[pos] = Object.Instantiate(displayFishPrefab, fishTankPos1);
			fishOnDisplay[pos].GetComponent<FishTankFish>().skinMeshAndSetAnimation(Inventory.Instance.getInvItemId(MuseumManager.manage.allFish[pos]), backTankSize);
		}
		else if (MuseumManager.manage.allFish[pos].fish.mySeason.myWaterLocation[0] == SeasonAndTime.waterLocation.Billabongs)
		{
			...
			fishOnDisplay[pos].GetComponent<FishTankFish>().skinMeshAndSetAnimation(Inventory.Instance.getInvItemId(MuseumManager.manage.allFish[pos]), desertTankSize);
		}
		...
		else
		{
			...
			fishOnDisplay[pos].GetComponent<FishTankFish>().skinMeshAndSetAnimation(Inventory.Instance.getInvItemId(MuseumManager.manage.allFish[pos]), middleTankSize);
		}
	}

	public void createUnderWaterDisplayAtPos(int pos)
	{
		if (MuseumManager.manage.allUnderWaterCreatures[pos].underwaterCreature.mySeason.myWaterLocation[0] == SeasonAndTime.waterLocation.NorthOcean || MuseumManager.manage.allUnderWaterCreatures[pos].underwaterCreature.mySeason.myWaterLocation[0] == SeasonAndTime.waterLocation.SouthOcean || MuseumManager.manage.allUnderWaterCreatures[pos].underwaterCreature.mySeason.myWaterLocation[0] == SeasonAndTime.waterLocation.ReefIsland)
		{
			underWaterCreaturesOnDisplay[pos] = Object.Instantiate(bugExhibitPrefab, fishTankPos1);
			underWaterCreaturesOnDisplay[pos].GetComponent<BugExhibit>().placeBugAndShowDisplay(Inventory.Instance.getInvItemId(MuseumManager.manage.allUnderWaterCreatures[pos]), backTankSize);
		}
		else if (MuseumManager.manage.allUnderWaterCreatures[pos].underwaterCreature.mySeason.myWaterLocation[0] == SeasonAndTime.waterLocation.Billabongs)
		{
			...
			underWaterCreaturesOnDisplay[pos].GetComponent<BugExhibit>().placeBugAndShowDisplay(Inventory.Instance.getInvItemId(MuseumManager.manage.allUnderWaterCreatures[pos]), desertTankSize);
		}
		...
		else
		{
			...
			underWaterCreaturesOnDisplay[pos].GetComponent<BugExhibit>().placeBugAndShowDisplay(Inventory.Instance.getInvItemId(MuseumManager.manage.allUnderWaterCreatures[pos]), middleTankSize);
		}
	}
	...
	public void createBugDisplaceAtPos(int pos)
	{
		if (MuseumManager.manage.allBugs[pos].bug.mySeason.myLandLocation[0] == SeasonAndTime.landLocation.Desert)
		{
			...
			bugsOnDisplay[pos].GetComponent<BugExhibit>().placeBugAndShowDisplay(Inventory.Instance.getInvItemId(MuseumManager.manage.allBugs[pos]), desertTankSize);
		}
		else if (MuseumManager.manage.allBugs[pos].bug.insectType.name.Contains("Flying") && MuseumManager.manage.allBugs[pos].bug.control == null)
		{
			...
			bugsOnDisplay[pos].GetComponent<BugExhibit>().placeBugAndShowDisplay(Inventory.Instance.getInvItemId(MuseumManager.manage.allBugs[pos]), middleTankSize);
		}
		else
		{
			bugsOnDisplay[pos] = Object.Instantiate(bugExhibitPrefab, backTank);
			bugsOnDisplay[pos].GetComponent<BugExhibit>().placeBugAndShowDisplay(Inventory.Instance.getInvItemId(MuseumManager.manage.allBugs[pos]), backTankSize);
		}
	}
}
