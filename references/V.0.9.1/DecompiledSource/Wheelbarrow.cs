...
public class Wheelbarrow : NetworkBehaviour
{
	...
	private void changeDirtTotal(int oldTotal, int newTotal)
	{
		...
		{
			SoundManager.Instance.playASoundAtPoint(WorldManager.Instance.tileTypes[topDirtId].onHeightUp, base.transform.position);
		}
		else if (topDirtId > -1)
		{
			SoundManager.Instance.playASoundAtPoint(WorldManager.Instance.tileTypes[topDirtId].onHeightDown, base.transform.position);
		}
	...
	private void updateTopLayer()
	{
		if (topDirtId > -1)
		{
			dirtFillUpRen.sharedMaterial = WorldManager.Instance.tileTypes[topDirtId].myTileMaterial;
		}
	}
	...
	public void updateContents()
	{
		if (totalDirt == 0)
		{
			dirtFillUp.gameObject.SetActive(value: false);
			return;
		}
		dirtFillUp.gameObject.SetActive(value: true);
