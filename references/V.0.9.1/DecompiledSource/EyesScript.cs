...
public class EyesScript : MonoBehaviour
{
	...
	private WaitForSeconds mouthWait = new WaitForSeconds(0.015f);
	...
	{
		if ((bool)myChar && (float)myChar.stamina <= 0.1f)
		{
			if (!tired)
			{
				tired = true;
	...
	private IEnumerator charBlinks()
	{
		...
		if ((object)CameraController.control != null)
		{
			while (Vector3.Distance(CameraController.control.transform.position, base.transform.position) > 50f)
			{
				yield return null;
				if (CharacterCreatorScript.create.gameObject.activeSelf)
				{
					break;
				}
			}
	...
	public void setFaceSigh()
	{
		if (playingCurrentEmotion != emotion.Sigh)
		{
			...
			SoundManager.Instance.playASoundAtPoint(SoundManager.Instance.sighSound, base.transform.root.position);
	...
	public void setFaceQuestion()
	{
		if (playingCurrentEmotion != emotion.Question)
		{
			...
			SoundManager.Instance.playASoundAtPoint(SoundManager.Instance.questionSound, base.transform.root.position);
	...
	public void setFaceShocked()
	{
		if (playingCurrentEmotion != emotion.Shocked)
		{
			...
			SoundManager.Instance.playASoundAtPoint(SoundManager.Instance.shockedSound, base.transform.root.position);
	...
	public void speakingWordsPart()
	{
		ParticleManager.manage.playerSpeakingPart.transform.GetChild(0).rotation = Quaternion.Euler(-90f, 0f, CameraController.control.transform.eulerAngles.y);
		ParticleManager.manage.emitParticleAtPosition(ParticleManager.manage.playerSpeakingPart, base.transform.position + Vector3.up + CameraController.control.mainCamera.transform.forward, 4);
	}
	...
	private IEnumerator faceGlee()
	{
		while (playingCurrentEmotion == emotion.Glee)
		{
			...
			SoundManager.Instance.playASoundAtPoint(SoundManager.Instance.gleeSound, base.transform.root.position);
	...
	{
		SoundManager.Instance.playASoundAtPoint(SoundManager.Instance.worriedSound, base.transform.root.position);
	...
	private IEnumerator faceProud()
	{
		...
		SoundManager.Instance.play2DSound(SoundManager.Instance.proudSound);
		currentEmotion = null;
	}
