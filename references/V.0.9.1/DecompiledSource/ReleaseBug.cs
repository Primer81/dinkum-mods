...
public class ReleaseBug : MonoBehaviour
{
	...
	public void setUpForBug(int bugId)
	{
		...
		if (Inventory.Instance.allItems[bugId].bug.insectType.name.Contains("Arach") || Inventory.Instance.allItems[bugId].bug.insectType.name.Contains("Hopping"))
		{
			gameObject = Object.Instantiate(Inventory.Instance.allItems[bugId].bug.insectType, crawlingBugTrans);
			StartCoroutine(crawlingBugRunAway());
		}
		else
		{
			gameObject = Object.Instantiate(Inventory.Instance.allItems[bugId].bug.insectType, flyingBugTrans);
			StartCoroutine(flyingBugRunAway());
		}
		gameObject.transform.localPosition = Vector3.zero;
		gameObject.transform.localRotation = Quaternion.identity;
		gameObject.GetComponentInChildren<Collider>().enabled = false;
		gameObject.GetComponent<BugAppearance>().setUpBug(Inventory.Instance.allItems[bugId]);
	...
	private IEnumerator crawlingBugRunAway()
	{
		...
		while (timer2 < 1f)
		{
			...
			position.y = WorldManager.Instance.heightMap[(int)(position.x / 2f), (int)(position.z / 2f)];
		...
		while (timer2 < 1f)
		{
			...
			position2.y = WorldManager.Instance.heightMap[(int)(position2.x / 2f), (int)(position2.z / 2f)];
	...
	private IEnumerator fishJumpsInWater()
	{
		...
		if (WorldManager.Instance.IsFishPondInPos(base.transform.position))
		{
			while (fishEscapeTrans.transform.position.y > (float)WorldManager.Instance.heightMap[Mathf.RoundToInt(base.transform.position.x / 2f), Mathf.RoundToInt(base.transform.position.z / 2f)] + 0.6f)
			{
			...
			ParticleManager.manage.bigSplash(fishEscapeTrans, 5, (float)WorldManager.Instance.heightMap[Mathf.RoundToInt(base.transform.position.x / 2f), Mathf.RoundToInt(base.transform.position.z / 2f)] + 0.61f);
		}
		else
		{
		...
		SoundManager.Instance.playASoundAtPoint(SoundManager.Instance.bigWaterSplash, fishEscapeTrans.position);
		float timer2 = 0f;
		while (timer2 < 1f)
		{
