...
public class FarmAnimal : NetworkBehaviour
{
	...
	public InventoryItem recieveOnSell;

	public GameObject jamesGlasses;
	...
	public void updateAnimalDetails(FarmAnimalDetails updateDetails)
	{
		...
		if ((bool)canBeHarvested && myDetails.ateYesterDay)
		{
			if (myDetails.hasHouse())
			{
	...
	{
		NetworkanimalName = newName;
		if ((bool)jamesGlasses)
		{
			if (animalName == "James" || animalName == "DinkumDev" || animalName == "Dinkum Dev" || animalName == "James Bendon")
			{
				jamesGlasses.SetActive(value: true);
			}
			else
			{
				jamesGlasses.SetActive(value: false);
			}
		}
	}

	public void renameAnimal(FarmAnimalDetails updateDetails)
	{
		NetworkanimalName = updateDetails.animalName;
		NetworkanimalAge = updateDetails.animalAge;
		NetworkanimalRelationship = updateDetails.animalRelationShip;
	...
	{
		NotificationManager.manage.createChatNotification(string.Format(ConversationGenerator.generate.GetToolTip("Tip_AnimalDied"), animalName));
		if (base.isServer)
		{
	...
	{
		NotificationManager.manage.createChatNotification(string.Format(ConversationGenerator.generate.GetToolTip("Tip_AnimalFaint"), animalName));
	}
	...
	public void RpcPetAnimal()
	{
		...
		SendRPCInternal(typeof(FarmAnimal), "RpcPetAnimal", writer, 0, includeOwner: true);
	...
	{
		_ = RealWorldTimeLight.time.isNightTime;
	}
	...
	private IEnumerator petTimer()
	{
		...
		{
			SoundManager.Instance.playASoundAtPoint(animalPatSound, base.transform.position);
		}
		if (!patParticlePos)
		{
	...
	public override void DeserializeSyncVars(NetworkReader reader, bool initialState)
	{
		...
		if (initialState)
		{
			...
			if (!SyncVarEqual(text, ref animalName))
			{
				OnChangeName(text, animalName);
			}
			int num = animalRelationship;
		...
		if ((num3 & 1L) != 0L)
		{
			...
			NetworkanimalName = reader.ReadString();
			if (!SyncVarEqual(text2, ref animalName))
			{
				OnChangeName(text2, animalName);
			}
		}
		if ((num3 & 2L) != 0L)
		{
