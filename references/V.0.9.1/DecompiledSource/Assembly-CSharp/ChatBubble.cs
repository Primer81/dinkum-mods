...
public class ChatBubble : MonoBehaviour
{
	...
	private IEnumerator killSelf()
	{
		...
		yield return StartCoroutine(fadeBackgroundAndText(fadeIn: false));
	...
	public void fillBubble(string name, string message)
	{
		...
		StartCoroutine(fadeBackgroundAndText(fadeIn: true));
	}
	...
	private IEnumerator fadeBackgroundAndText(bool fadeIn)
	{
		...
		while (timer < 1f)
		{
			if (fadeIn)
			{
				Material[] fontMaterials = contents.fontMaterials;
				for (int i = 0; i < fontMaterials.Length; i++)
				{
					fontMaterials[i].SetColor("_FaceColor", Color.Lerp(Color.clear, Color.white, timer));
				}
			...
			else
			{
				...
				for (int i = 0; i < fontMaterials.Length; i++)
				{
					fontMaterials[i].SetColor("_FaceColor", Color.Lerp(Color.white, Color.clear, timer));
				}
