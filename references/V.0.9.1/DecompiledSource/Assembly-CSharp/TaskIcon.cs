...
public class TaskIcon : MonoBehaviour
{
	...
	public void OnEnable()
	{
		...
		{
			completedTick.SetActive(value: true);
		}
		else
		{
			completedTick.SetActive(value: false);
		}
	}
	...
	public void fillWithDetails(Task task)
	{
		...
		trackingTask = task;
		TaskText.text = task.GetMissionText();
		points = task.points;
	...
	{
		completedTick.SetActive(value: true);
		SoundManager.Instance.play2DSound(completeSound);
		PermitPointsManager.manage.addPoints(trackingTask.reward);
	}

	public void makeSmall()
	{
		taskDescBox.SetActive(value: false);
		rewardBox.gameObject.SetActive(value: false);
		GetComponent<RectTransform>().sizeDelta = new Vector2(40f, 40f);
	}

	public void makeBig()
	{
		taskDescBox.SetActive(value: true);
		rewardBox.gameObject.SetActive(value: true);
