...
public class FishScript : NetworkBehaviour
{
	...
	{
		int fakeBites = 0;
		animalAnim.makeFishInterested(newInterested: true);
		...
		animalAnim.makeFishInterested(newInterested: false);
	...
	private void RpcFishBite()
	{
		...
		SendRPCInternal(typeof(FishScript), "RpcFishBite", writer, 0, includeOwner: true);
	...
	private void RpcFakeFishBite(uint rodId)
	{
		...
		SendRPCInternal(typeof(FishScript), "RpcFakeFishBite", writer, 0, includeOwner: true);
		NetworkWriterPool.Recycle(writer);
	}
