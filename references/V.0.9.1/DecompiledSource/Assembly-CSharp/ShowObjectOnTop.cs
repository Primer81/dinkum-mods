...
public class ShowObjectOnTop : MonoBehaviour
{
	...
	public void getPrefabForPos(ItemOnTop itemOnTopToSpawn)
	{
		...
		{
			showingTileObjectOnTop[itemOnTopToSpawn.sittingOnTopPosition()] = WorldManager.Instance.getTileObjectForOnTop(itemOnTopToSpawn.getTileObjectId(), onTopPositions[itemOnTopToSpawn.sittingOnTopPosition()].transform.position);
			showingTileObjectOnTop[itemOnTopToSpawn.sittingOnTopPosition()].placeDown();
			showingTileObjectOnTop[itemOnTopToSpawn.sittingOnTopPosition()].xPos = itemOnTopToSpawn.sittingOnX;
			showingTileObjectOnTop[itemOnTopToSpawn.sittingOnTopPosition()].yPos = itemOnTopToSpawn.sittingOnY;
		}
	...
	private void OnDisable()
	{
		for (int i = 0; i < showingTileObjectOnTop.Length; i++)
		{
			if (showingTileObjectOnTop[i] != null)
			{
				showingTileObjectOnTop[i].gameObject.SetActive(value: false);
			}
		}
	}
	...
	private void OnEnable()
	{
		for (int i = 0; i < showingTileObjectOnTop.Length; i++)
		{
			if (showingTileObjectOnTop[i] != null)
			{
				showingTileObjectOnTop[i].gameObject.SetActive(value: true);
			}
		}
	}
}
