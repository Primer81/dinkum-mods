...
public class MetalDetectorUse : MonoBehaviour
{
	...
	{
		return new Vector3(lastChecked.x * 2f, WorldManager.Instance.heightMap[(int)lastChecked.x, (int)lastChecked.y], lastChecked.y * 2f);
	}
	...
	private void Update()
	{
		...
		if ((checking && WorldManager.Instance.onTileMap[xPos, yPos] == 30) || (checking && WorldManager.Instance.tileTypeMap[xPos, yPos] == 22))
		{
			...
			{
				partTimer = 0f;
				ParticleManager.manage.emitParticleAtPosition(ParticleManager.manage.metalDetectorFound, new Vector3(xPos * 2, (float)WorldManager.Instance.heightMap[xPos, yPos] + 0.1f, yPos * 2), 1);
			}
			noiseVolume = 0.75f;
			if (myCharInteract.isLocalPlayer)
			{
		...
		else if (checking && inSurroundingTiles(xPos, yPos))
		{
			...
			{
				partTimer = 0f;
				ParticleManager.manage.emitParticleAtPosition(ParticleManager.manage.metalDetectorClose, new Vector3(xPos * 2, (float)WorldManager.Instance.heightMap[xPos, yPos] + 0.1f, yPos * 2), 1);
			}
			noiseVolume = 0.4f;
		}
		else
		...
		metalDetectorSource.volume = Mathf.Lerp(metalDetectorSource.volume, noiseVolume * SoundManager.Instance.GetGlobalSoundVolume(), Time.deltaTime * 10f);
	}
	...
	{
		if (xPos == 0 || yPos == 0 || xPos == WorldManager.Instance.GetMapSize() - 1 || yPos == WorldManager.Instance.GetMapSize() - 1)
		{
			return false;
		}
		if (WorldManager.Instance.onTileMap[xPos + 1, yPos] == 30)
		{
			return true;
		}
		if (WorldManager.Instance.onTileMap[xPos - 1, yPos] == 30)
		...
		if (WorldManager.Instance.onTileMap[xPos, yPos + 1] == 30)
		{
			return true;
		}
		if (WorldManager.Instance.onTileMap[xPos, yPos - 1] == 30)
		...
		if (WorldManager.Instance.onTileMap[xPos + 1, yPos + 1] == 30)
		{
			return true;
		}
		if (WorldManager.Instance.onTileMap[xPos + 1, yPos - 1] == 30)
