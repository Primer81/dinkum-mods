...
public class HangGliderHandObject : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (isBoogieBoard && (bool)myMove)
		{
			...
			myMove.changeSwimSpeedItem(swimDif);
	...
	private void FixedUpdate()
	{
		...
		{
			if ((myMove.isLocalPlayer && myMove.localUsing) || (!myMove.isLocalPlayer && myMove.myEquip.usingItem))
			{
		...
		bool flag = myMove.myEquip.usingItem;
		if (myMove.isLocalPlayer)
		{
			flag = myMove.localUsing;
		}
		gliderAnimator.SetBool("Using", flag);
		if (flag)
		{
			emitParts();
		}
		...
		if ((bool)flyingAudio && flag && !gliderAnimator.GetBool("Grounded"))
		{
			if ((bool)flyingAudio)
			{
				...
				flyingAudio.volume = 0.25f * SoundManager.Instance.GetGlobalSoundVolume();
			}
		}
	...
	public void getDaysDif()
	{
		...
		if (WeatherManager.Instance.windMgr.IsActive)
		{
			speedDif = 3;
			jumpDif = 4;
		}
	...
	private IEnumerator useStamina()
	{
		...
		{
			yield return null;
			if (myMove.localUsing && !myMove.grounded)
			{
