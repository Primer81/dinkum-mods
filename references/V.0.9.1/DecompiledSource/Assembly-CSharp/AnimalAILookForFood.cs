...
public class AnimalAILookForFood : NetworkBehaviour
{
	...
	public IEnumerator searchForFoodNearby()
	{
		...
		if (loopsSinceLastCheck < Random.Range(8, 12))
		{
			...
			if (isHungry && Random.Range(0, 15) == 7)
			{
				...
				{
					currentlyTryingToEatPos = WorldManager.Instance.findDroppedObjectAround(base.transform.position, eatsDrops, dropCheckDistance);
				...
				{
					currentlyTryingToEatPos = WorldManager.Instance.findClosestTileObjectToPosition(base.transform.position, eatsTiles, 15, checkIfFencedOff: true);
				}
				if (animalEatsCrops && !isEatingADrop && currentlyTryingToEatPos == Vector3.zero)
				{
					currentlyTryingToEatPos = WorldManager.Instance.findClosestTileObjectToPosition(base.transform.position, TownManager.manage.allCropsTypes, 5, checkIfFencedOff: true);
				}
				if (!(currentlyTryingToEatPos != Vector3.zero))
				{
					yield break;
				}
				int objectEatingId = WorldManager.Instance.onTileMap[(int)currentlyTryingToEatPos.x / 2, (int)currentlyTryingToEatPos.z / 2];
				...
				while (isHungry && currentlyTryingToEatPos != Vector3.zero && !myAi.returnClosestEnemy() && myAi.currentlyAttacking() == null)
				{
					...
					else
					{
						...
						if (currentlyTryingToEatPos != Vector3.zero)
						{
							objectEatingId = WorldManager.Instance.onTileMap[(int)currentlyTryingToEatPos.x / 2, (int)currentlyTryingToEatPos.z / 2];
							myAi.myAgent.SetDestination(currentlyTryingToEatPos);
						}
					}
	...
	private IEnumerator EatFood(Vector3 currentlyTryingToEatPos, int objectEatingId)
	{
		...
		if (!isEatingADrop)
		{
			eatingHealth = (int)WorldManager.Instance.allObjectSettings[objectEatingId].fullHealth;
		}
		while (myAi.checkIfShouldContinue() && isWhatIWantStillThere(currentlyTryingToEatPos, objectEatingId) && currentlyTryingToEatPos != Vector3.zero && !myAi.returnClosestEnemy())
		{
			if (isEatingADrop)
			{
				List<DroppedItem> allDropsOnTile = WorldManager.Instance.getAllDropsOnTile((int)currentlyTryingToEatPos.x / 2, (int)currentlyTryingToEatPos.z / 2);
				bool flag = false;
				for (int i = 0; i < allDropsOnTile.Count; i++)
				{
					for (int j = 0; j < eatsDrops.Length; j++)
					{
						if (allDropsOnTile[i].myItemId == Inventory.Instance.getInvItemId(eatsDrops[j]))
						{
			...
			else
			{
				...
				if (eatingHealth <= 0)
				{
					if ((bool)WorldManager.Instance.allObjects[objectEatingId].tileOnOff)
					{
						NetworkMapSharer.Instance.RpcOpenCloseTile((int)currentlyTryingToEatPos.x / 2, (int)currentlyTryingToEatPos.z / 2, 0);
					}
					else
					{
						NetworkMapSharer.Instance.RpcUpdateOnTileObject(-1, (int)currentlyTryingToEatPos.x / 2, (int)currentlyTryingToEatPos.z / 2);
					}
					currentlyTryingToEatPos = Vector3.zero;
					checkIfStillHungry();
				}
	...
	private bool isWhatIWantStillThere(Vector3 wantToEatPos, int objectEatingId)
	{
		if (isEatingADrop)
		{
			List<DroppedItem> allDropsOnTile = WorldManager.Instance.getAllDropsOnTile((int)wantToEatPos.x / 2, (int)wantToEatPos.z / 2);
			for (int i = 0; i < allDropsOnTile.Count; i++)
			{
				for (int j = 0; j < eatsDrops.Length; j++)
				{
		...
		if ((WorldManager.Instance.onTileMap[(int)wantToEatPos.x / 2, (int)wantToEatPos.z / 2] > -1 && !WorldManager.Instance.allObjects[objectEatingId].tileOnOff) || ((bool)WorldManager.Instance.allObjects[objectEatingId].tileOnOff && WorldManager.Instance.onTileStatusMap[(int)wantToEatPos.x / 2, (int)wantToEatPos.z / 2] == 1))
		{
			return true;
		}
		return false;
	...
	private void checkIfStillHungry()
	{
		...
		if ((bool)isFarmAnimal)
		{
			...
			isFarmAnimal.setEaten(hasEaten: true);
		}
	}
	...
	private void RpcEat(int xPos, int yPos)
	{
		...
		SendRPCInternal(typeof(AnimalAILookForFood), "RpcEat", writer, 0, includeOwner: true);
	...
	{
		PooledNetworkWriter writer = NetworkWriterPool.GetWriter();
		SendRPCInternal(typeof(AnimalAILookForFood), "RpcEatADrop", writer, 0, includeOwner: true);
		NetworkWriterPool.Recycle(writer);
	}
