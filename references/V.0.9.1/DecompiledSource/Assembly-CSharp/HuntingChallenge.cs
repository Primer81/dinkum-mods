...
public class HuntingChallenge
{
	...
	public void generatePosition(int desiredBiomeId)
	{
		...
		if (num3 == 0)
		{
			Debug.Log("Tried 100000 times and couldn't find the biome");
		}
		int num4 = challengeAnimalId;
		int num5 = num;
		int i = num2;
		for (bool canSpawnInWater = AnimalManager.manage.allAnimals[num4].saveAsAlpha.canSpawnInWater; (WorldManager.Instance.isPositionOnMap(num5, i) && !canSpawnInWater && WorldManager.Instance.waterMap[num5, i]) || !AnimalManager.manage.checkIfTileIsWalkable(num5, i); i += UnityEngine.Random.Range(-1, 2))
		{
		...
		{
			num5 * 2,
			WorldManager.Instance.heightMap[num, num2],
			i * 2
		};
		spawnChunkX = Mathf.RoundToInt(num / WorldManager.Instance.getChunkSize()) * WorldManager.Instance.getChunkSize();
		spawnChunkY = Mathf.RoundToInt(num2 / WorldManager.Instance.getChunkSize()) * WorldManager.Instance.getChunkSize();
	}
	...
	{
		return AnimalManager.manage.allAnimals[challengeAnimalId].GetAnimalName();
	}
