...
public class AnimalMakeSounds : MonoBehaviour
{
	...
	public IEnumerator makeSoundForBox()
	{
		...
		while (true)
		{
			...
			if (!isPlayingOtherSound && Random.Range(0, 8) == 2)
			{
				...
				{
					Invoke("whenSoundFinished", SoundManager.Instance.playASoundAtPointAndReturnLength(animalSoundPool, base.transform.position, out currentlyPlaying, 1f, pitchDif));
				}
				else
				{
					Invoke("whenSoundFinished", SoundManager.Instance.playASoundAtPointAndReturnLength(animalSoundPool, base.transform.position, out currentlyPlaying));
				}
			}
		}
	}
	...
	public void playDamageSound()
	{
		...
		if ((bool)hasVariation)
		{
			Invoke("whenSoundFinished", SoundManager.Instance.playASoundAtPointAndReturnLength(animalDamageSound, base.transform.position, out currentlyPlaying, 1f, pitchDif));
		}
		else
		{
			Invoke("whenSoundFinished", SoundManager.Instance.playASoundAtPointAndReturnLength(animalDamageSound, base.transform.position, out currentlyPlaying));
		}
	}
	...
	public void playAnimalSoundOnAttack()
	{
		...
		if ((bool)hasVariation)
		{
			Invoke("whenSoundFinished", SoundManager.Instance.playASoundAtPointAndReturnLength(attackSound, base.transform.position, out currentlyPlaying, 1f, pitchDif));
		}
		else
		{
			Invoke("whenSoundFinished", SoundManager.Instance.playASoundAtPointAndReturnLength(attackSound, base.transform.position, out currentlyPlaying));
		}
	}
}
