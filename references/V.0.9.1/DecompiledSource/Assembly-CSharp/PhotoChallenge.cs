...
public class PhotoChallenge
{
	...
	public void randomiseRequirements()
	{
		...
		else if (subjectType == 3)
		{
			...
			{
				num2,
				WorldManager.Instance.heightMap[num2 / 2, num3 / 2],
				num3
			};
		...
		else if (subjectType == 2)
		{
			...
			while (!flag)
			{
				if (SaveLoad.saveOrLoad.carryablePrefabs[num4].GetComponent<PickUpAndCarry>().photoRequestable && (bool)SaveLoad.saveOrLoad.carryablePrefabs[num4].GetComponent<SellByWeight>())
				{
					flag = true;
				}
				else
