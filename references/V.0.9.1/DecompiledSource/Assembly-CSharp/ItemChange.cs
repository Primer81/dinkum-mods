...
public class ItemChange : MonoBehaviour
{
	...
	public int getChangerResultId(int itemChangerId)
	{
		...
		foreach (ItemChangeType itemChangeType in array)
		{
			if (itemChangeType.depositInto.tileObjectId == itemChangerId)
			{
				...
				{
					return Inventory.Instance.getInvItemId(itemChangeType.changesWhenCompleteTable.getRandomDropFromTable());
				}
				return Inventory.Instance.getInvItemId(itemChangeType.changesWhenComplete);
			}
		}
		return -1;
	}
	...
	public bool checkIfCanBeDeposited(int itemChangerId)
	{
		...
		foreach (ItemChangeType itemChangeType in array)
		{
			if (itemChangeType.depositInto.tileObjectId == itemChangerId && Inventory.Instance.getAmountOfItemInAllSlots(Inventory.Instance.getInvItemId(GetComponent<InventoryItem>())) >= itemChangeType.amountNeededed)
			{
				return true;
			}
		}
	...
	public void checkTask(int itemChangerId)
	{
		...
		foreach (ItemChangeType itemChangeType in array)
		{
			...
			if (itemChangeType.givesXp)
			{
				...
				{
					CharLevelManager.manage.addToDayTally(Inventory.Instance.getInvItemId(itemChangeType.changesWhenComplete), 1, (int)itemChangeType.xPType);
				}
			}
		}
	}
