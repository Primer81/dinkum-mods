...
public class FarmAnimalHarvest : NetworkBehaviour
{
	...
	public string getToolTip(InventoryItem itemInHand)
	{
		if (harvestReady && (!requiredToHarvest || requiredToHarvest == itemInHand))
		{
			if (harvestVerb.Contains("Tip_"))
			{
				return string.Format(ConversationGenerator.generate.GetToolTip(harvestVerb), myAnimal.getAnimalName());
			}
	...
	public void onAnimalHarvest(bool old, bool newHarvest)
	{
		...
		if (!newHarvest)
		{
			...
			{
				SoundManager.Instance.playASoundAtPoint(harvestSound, base.transform.position);
			}
	...
	{
		if (!Inventory.Instance.addItemToInventory(itemId, 1))
		{
			NetworkMapSharer.Instance.localChar.CmdDropItem(itemId, 1, base.transform.position, base.transform.position);
		}
	}
