...
public class FishingRodCastAndReel : MonoBehaviour
{
	...
	public void cancelCastCompleteOnServer()
	{
		castComplete = false;
	}
	...
	private void Update()
	{
		...
		if (fishPulling || (networkVersion.fishOnLine != -1 && !fishPulling && !myChar.myEquip.usingItem))
		{
			...
			{
				SoundManager.Instance.playASoundAtPoint(SoundManager.Instance.waterSplash, bobber.position, 0.35f);
		...
		if (!bobber.gameObject.activeSelf || !Physics.Raycast(bobber.transform.position + Vector3.up * 3f, Vector3.down, out var hitInfo, 25f, castMask))
		{
			return;
		}
	...
	{
		if (myChar.isLocalPlayer && Physics.Raycast(networkVersion.transform.position + networkVersion.transform.forward * castDistance + Vector3.up * 3f, Vector3.down, out var hitInfo, 25f, castMask))
		{
	...
	private void getNewReelPos()
	{
		...
		if (Physics.Raycast(bobber.position + normalized * num + Vector3.up * 3f, Vector3.down, out var hitInfo, 25f, castMask))
		{
	...
	private void OnDisable()
	{
		if ((bool)networkVersion && networkVersion.isLocalPlayer)
		{
			...
			completeReel(beingDisabled: true);
			networkVersion.CmdCancleCast();
			CameraController.control.stopFishing();
			networkVersion.lineIsCasted = false;
			if (networkVersion.connectedFishingRod == this)
			{
				networkVersion.connectedFishingRod = null;
			}
		}
		if ((bool)networkVersion && networkVersion.isLocalPlayer)
		{
	...
	public void stopDummyStruggle()
	{
		...
		{
			myFishDummyAnim.SetBool("Struggle", value: false);
			myFishDummyAnim.SetBool("Walking", value: true);
		}
	}
	...
	public void dummyStruggle()
	{
		if ((bool)myFishDummyAnim)
		{
			myFishDummyAnim.SetBool("Struggle", value: true);
		}
	}
	...
	public void putAwayFish()
	{
		...
		{
			if (networkVersion.WasLastCaughtSparkling())
			{
				RandomObjectGenerator.generate.GiveRandomFishingReward();
			}
			myChar.myEquip.catchAndShowFish(myFishDummy.GetComponent<FishType>().getFishInvId(), networkVersion.WasPulledFromPond());
		}
	}
	...
	public void completeReel(bool beingDisabled = false)
	{
		...
		if (!beingDisabled)
		{
			if (Physics.Raycast(bobber.position + Vector3.up * 3f, Vector3.down, out var hitInfo, 25f, castMask) && hitInfo.transform.gameObject.layer == LayerMask.NameToLayer("Water"))
			{
				SoundManager.Instance.playASoundAtPoint(SoundManager.Instance.waterSplash, bobber.position, 0.8f);
		...
		bobber.parent = RodEnd;
		bobber.gameObject.SetActive(value: false);
		if ((bool)myFishDummy)
		{
		...
		if (networkVersion.fishOnLine != -1 && !beingDisabled && WorldManager.Instance.isPositionOnMap(bobberDummy.position) && WorldManager.Instance.waterMap[Mathf.RoundToInt(bobberDummy.position.x / 2f), Mathf.RoundToInt(bobberDummy.position.z / 2f)])
		{
			Vector3 position = bobberDummy.position;
			position.y = 0.6f;
			SoundManager.Instance.playASoundAtPoint(SoundManager.Instance.waterSplash, position);
	...
	private IEnumerator castLineToPosition()
	{
		...
		bobber.gameObject.SetActive(value: true);
		...
		while (!castComplete)
		{
			...
			if (Physics.Raycast(networkVersion.transform.position + networkVersion.transform.forward * castDistance + Vector3.up * 3f, Vector3.down, out var hitInfo, 25f, castMask))
			{
				if (hitInfo.transform.gameObject.layer == LayerMask.NameToLayer("Water"))
				{
					bobberAnim.SetTrigger("Landed");
	...
	{
		reelSounds.volume = 0.36f * SoundManager.Instance.GetGlobalSoundVolume();
	...
	public void playReelSound(float pitch)
	{
		if (networkVersion.fishOnLine != -1)
		{
			...
			{
				pitch *= 2.5f;
				reelSounds.volume = 0.56f * SoundManager.Instance.GetGlobalSoundVolume();
			}
			else
			{
				pitch *= 1.5f;
				reelSounds.volume = 0.46f * SoundManager.Instance.GetGlobalSoundVolume();
			}
		}
		else
		{
	...
	public void playChargeSound(float pitch)
	{
		...
		chargeSounds.volume = 0.25f * SoundManager.Instance.GetGlobalSoundVolume();
	}
	...
	{
		float timer2 = 0.5f;
		reelSounds.volume = 0.8f * SoundManager.Instance.GetGlobalSoundVolume();
		while (timer2 > 0f)
		{
		...
		while (timer2 > 0f)
		{
			reelSounds.volume = Mathf.Lerp(0f, 0.8f * SoundManager.Instance.GetGlobalSoundVolume(), timer2 * 4f);
			timer2 -= Time.deltaTime;
			yield return null;
		}
		stopReelSounds();
