...
public class AnimalAI_Pet : NetworkBehaviour
{
			...
			if ((bool)followingTransform && !myAi.isDead())
			{
				if (myAi.myAgent.isActiveAndEnabled && myAi.myAgent.isOnNavMesh)
				{
					...
					while (myAi.myAgent.isActiveAndEnabled && myAi.myAgent.isOnNavMesh && Mathf.Abs(followingTransform.position.y - myAi.myAgent.transform.position.y) >= 1.85f && connectedChar.grounded && !boolCheckIfCanRunToChar() && timer < 3f)
					{
						...
						destination.y = WorldManager.Instance.heightMap[(int)destination.x / 2, (int)destination.z / 2];
						...
						checkPos.y = WorldManager.Instance.heightMap[(int)checkPos.x / 2, (int)checkPos.z / 2];
					...
					if (myAi.myAgent.isActiveAndEnabled && myAi.myAgent.isOnNavMesh)
					{
						if (distance > 12f)
						{
							...
							else
							{
								...
								normalized = new Vector3(normalized.x, WorldManager.Instance.heightMap[(int)normalized.x / 2, (int)normalized.z / 2], normalized.z);
				...
				else
				{
					...
					normalized2 = new Vector3(normalized2.x, WorldManager.Instance.heightMap[(int)normalized2.x / 2, (int)normalized2.z / 2], normalized2.z);
	...
	public void checkForTileObjects()
	{
		if (currentlyLookingForPos == Vector3.zero)
		{
			...
			{
				currentlyLookingForPos = WorldManager.Instance.findTileObjectAround(base.transform.position, findTileObject);
			}
		}
		if (!(currentlyLookingForPos != Vector3.zero))
		{
		...
		if (!WorldManager.Instance.waterMap[Mathf.RoundToInt(currentlyLookingForPos.x / 2f), Mathf.RoundToInt(currentlyLookingForPos.z / 2f)])
		{
			...
			{
				myAi.myAgent.SetDestination(new Vector3(currentlyLookingForPos.x, WorldManager.Instance.heightMap[(int)currentlyLookingForPos.x / 2, (int)currentlyLookingForPos.z / 2], currentlyLookingForPos.z));
			}
			if (WorldManager.Instance.onTileMap[(int)currentlyLookingForPos.x / 2, (int)currentlyLookingForPos.z / 2] != findTileObject[0].tileObjectId)
			{
				stopAllTracking();
			}
		}
	...
	private void RpcStopSignal()
	{
		...
		SendRPCInternal(typeof(AnimalAI_Pet), "RpcStopSignal", writer, 0, includeOwner: true);
	...
	{
		PooledNetworkWriter writer = NetworkWriterPool.GetWriter();
		SendRPCInternal(typeof(AnimalAI_Pet), "RpcStartSignal", writer, 0, includeOwner: true);
		NetworkWriterPool.Recycle(writer);
	}
	...
	public bool boolCheckIfCanRunToChar()
	{
		...
		vector.y = WorldManager.Instance.heightMap[(int)base.transform.position.x / 2, (int)base.transform.position.z / 2];
		if (NavMesh.SamplePosition(followingTransform.position, out var hit, 1.5f, myAi.myAgent.areaMask) && myAi.myAgent.isActiveAndEnabled && myAi.myAgent.isOnNavMesh)
		{
	...
	{
		PooledNetworkWriter writer = NetworkWriterPool.GetWriter();
		SendRPCInternal(typeof(AnimalAI_Pet), "RpcPlayJump", writer, 0, includeOwner: true);
		NetworkWriterPool.Recycle(writer);
	}
	...
	{
		GetComponent<Animator>().SetBool("Signal", value: false);
	}
	...
	{
		GetComponent<Animator>().SetBool("Signal", value: true);
	}
