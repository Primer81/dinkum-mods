...
public class GenerateMap : MonoBehaviour
{
	...
	public BiomSpawnTable desertRainGrowBack;
	...
	private string[] getBiomeNameForMap = new string[19]
	{
		"Beach", "Tropics", "Billabong", "Billabong", "Bushlands", "Desert", "Pine Forest", "Plains", "Pine Forest", "Ocean",
		"Ocean", "Ocean", "Rough Soil", "Tropics", "Pine Forest", "Bushlands", "Beach", "Tropics", "Unknown"
	};
	...
	private void Start()
	{
		originalSpawnPoint = new Vector3(WorldManager.Instance.GetMapSize() / 2, 2f, WorldManager.Instance.GetMapSize() / 2);
		StartCoroutine(startDelay());
	}

	private IEnumerator logoTimer()
	{
		jamesLogo.SetActive(value: true);
		yield return new WaitForSeconds(3.5f);
		jamesLogo.SetActive(value: false);
	}

	public IEnumerator startDelay()
	{
		SaveLoad.saveOrLoad.loadingScreen.appear("Tip_Loading");
	...
	public void onFileLoaded()
	{
		...
		{
			WorldManager.Instance.spawnPos.position = TownManager.manage.lastSavedPos + Vector3.up;
		}
		else if (TownManager.manage.lastSleptPos != Vector3.zero)
		{
			WorldManager.Instance.spawnPos.position = TownManager.manage.lastSleptPos + Vector3.up;
		}
		UnityEngine.Random.InitState(Environment.TickCount);
	}
	...
	{
		int num = WorldManager.Instance.GetMapSize() / WorldManager.Instance.getChunkSize();
		for (int i = 0; i < num; i++)
		{
			for (int j = 0; j < num; j++)
			{
				...
				for (int k = 0; k < 10; k++)
				{
					for (int l = 0; l < 10; l++)
					{
						if (WorldManager.Instance.onTileMap[j * 10 + l, i * 10 + k] < 0)
						{
							continue;
						}
						if (WorldManager.Instance.onTileMap[j * 10 + l, i * 10 + k] == 44)
						{
							WorldManager.Instance.spawnPos.position = new Vector3((j * 10 + l) * 2, WorldManager.Instance.heightMap[j, i] + 10, (i * 10 + k) * 2);
							if (WorldManager.Instance.rotationMap[j * 10 + l, i * 10 + k] == 1)
							{
								WorldManager.Instance.spawnPos.position += new Vector3(0f, 0f, -2f);
							}
							else if (WorldManager.Instance.rotationMap[j * 10 + l, i * 10 + k] == 2)
							{
								WorldManager.Instance.spawnPos.position += new Vector3(-2f, 0f, 0f);
							}
							else if (WorldManager.Instance.rotationMap[j * 10 + l, i * 10 + k] == 3)
							{
								WorldManager.Instance.spawnPos.position += new Vector3(0f, 0f, 2f);
							}
							else
							...
							originalSpawnPoint = WorldManager.Instance.spawnPos.position;
							continue;
						}
						for (int m = 0; m < buildingsToSpawn.Length; m++)
						{
							if (WorldManager.Instance.onTileMap[j * 10 + l, i * 10 + k] == buildingsToSpawn[m].tileObjectId)
							{
	...
	public IEnumerator generateNewMap(int inSeed)
	{
		...
		for (int y = 0; y < mapSize; y++)
		{
			...
			{
				WorldManager.Instance.tileTypeMap[i, y] = 0;
				WorldManager.Instance.onTileMap[i, y] = -1;
				WorldManager.Instance.tileTypeStatusMap[i, y] = -1;
				WorldManager.Instance.onTileStatusMap[i, y] = -1;
				WorldManager.Instance.waterMap[i, y] = false;
				...
				WorldManager.Instance.heightMap[i, y] = 1;
				if (num > 0f)
				{
					if (num < 0.23f)
					{
						...
						{
							WorldManager.Instance.tileTypeMap[i, y] = 3;
							if (num < 0.08f)
							{
					...
					else if (tropicalClimate.getNoise(i, y) - num2 > 0f || num2 < 0.35f)
					{
						...
						WorldManager.Instance.heightMap[i, y] += (int)(heightTropicalClimate.getNoise(i, y) * 9f);
						if (cassowaryNests.getNoise(i, y) > 0.77f)
						{
							if (biomGrass.getNoise(i, y) < 0.77f)
							{
								WorldManager.Instance.tileTypeMap[i, y] = 4;
							}
						...
						{
							WorldManager.Instance.tileTypeMap[i, y] = 18;
					...
					else if (bushLandClimate.getNoise(i, y) - num3 > 0f || num3 < 0.35f)
					{
						...
						WorldManager.Instance.heightMap[i, y] += (int)(heightBooshLand.getNoise(i, y) * 3f);
						if (getBillabongHeight(i, y) != -5)
						{
							...
							WorldManager.Instance.heightMap[i, y] = getBillabongHeight(i, y);
							if (WorldManager.Instance.heightMap[i, y] > 0 && biomGrass.getNoise(i, y) < 0.75f)
							{
								WorldManager.Instance.tileTypeMap[i, y] = 1;
						...
						else if (dingoDens.getNoise(i, y) > 0.78f)
						{
							...
							{
								WorldManager.Instance.heightMap[i, y] = Mathf.Clamp(WorldManager.Instance.heightMap[i, y] - 1, 1, 4);
					...
					{
						WorldManager.Instance.heightMap[i, y] += (int)(heighBiomDesert.getNoise(i, y) * 2f);
						WorldManager.Instance.tileTypeMap[i, y] = 5;
						...
						{
							WorldManager.Instance.onTileMap[i, y] = 13;
						}
					...
					else if (coldClimate.getNoise(i, y) - num4 > 0f || num4 < 0.45f)
					{
						...
						WorldManager.Instance.heightMap[i, y] += (int)(heightTropicalClimate.getNoise(i, y) * 5f + heightBooshLand.getNoise(i, y) * 5f - num4 * 3f);
						if ((double)bunyipDens.getNoise(i, y) > 0.79)
						{
							WorldManager.Instance.tileTypeMap[i, y] = 18;
							if (bunyipDens.getNoise(i, y) > 0.82f)
							{
								WorldManager.Instance.heightMap[i, y]--;
								placeWorldObject(i, y, boneObjects);
							}
							else
							{
						...
						{
							WorldManager.Instance.tileTypeMap[i, y] = 15;
					...
					if (posRiverHeight != -5)
					{
						if (posRiverHeight != -4 && WorldManager.Instance.heightMap[i, y] > posRiverHeight)
						{
							WorldManager.Instance.heightMap[i, y] = posRiverHeight;
						}
						if (posRiverHeight == -4 && WorldManager.Instance.heightMap[i, y] > 1)
						{
							WorldManager.Instance.heightMap[i, y] = 1;
						}
						if (posRiverHeight != -4)
						{
							if (posRiverHeight <= 0)
							{
								WorldManager.Instance.waterMap[i, y] = true;
							}
					...
					{
						WorldManager.Instance.heightMap[i, y] = Mathf.Clamp(WorldManager.Instance.heightMap[i, y], -5, 1);
					}
					continue;
				}
				...
				if (num > -0.065f)
				{
					WorldManager.Instance.heightMap[i, y] = 0;
				}
				else if (num > -0.15f)
				{
					WorldManager.Instance.heightMap[i, y] = -1;
				...
				{
					WorldManager.Instance.heightMap[i, y] = -1 + (int)(heightBooshLand.getNoise(i, y) * -4f);
				}
				if (posRiverHeight2 != -5 && posRiverHeight2 < 0 && posRiverHeight2 != -4 && WorldManager.Instance.heightMap[i, y] > posRiverHeight2)
				{
					WorldManager.Instance.heightMap[i, y] = posRiverHeight2;
				}
				if (coldClimate.getNoise(i, y) - num4 * 3.5f > 0f)
				{
					...
					{
						WorldManager.Instance.heightMap[i, y] = 4;
				...
				else if (heightTropicalClimate.getNoise(i, y) - num5 * 3.8f > 0f)
				{
					if (heightTropicalClimate.getNoise(i, y) > 0.65f)
					{
						...
						{
							WorldManager.Instance.heightMap[i, y] = 2;
							WorldManager.Instance.tileTypeMap[i, y] = 4;
				...
				if (WorldManager.Instance.heightMap[i, y] < 0)
				{
		...
		{
			if (WorldManager.Instance.allObjects[multiTiledObjectsPlaceAfterMap[j][2]].checkIfMultiTileObjectCanBePlacedMapGenerate(multiTiledObjectsPlaceAfterMap[j][0], multiTiledObjectsPlaceAfterMap[j][1], multiTiledObjectsPlaceAfterMap[j][3]))
			{
				WorldManager.Instance.allObjects[multiTiledObjectsPlaceAfterMap[j][2]].placeMultiTiledObject(multiTiledObjectsPlaceAfterMap[j][0], multiTiledObjectsPlaceAfterMap[j][1], multiTiledObjectsPlaceAfterMap[j][3]);
			}
		}
		placeAnimalDens();
		WorldManager.Instance.chunksToRefresh.Clear();
	...
	public void placeWorldObject(int xPos, int yPos, BiomSpawnTable spawnFrom)
	{
		...
		if (biomObject != -1 && WorldManager.Instance.onTileMap[xPos, yPos] == -1)
		{
			if ((bool)WorldManager.Instance.allObjects[biomObject].tileObjectGrowthStages)
			{
				WorldManager.Instance.onTileStatusMap[xPos, yPos] = WorldManager.Instance.allObjects[biomObject].tileObjectGrowthStages.objectStages.Length - 1;
			}
			...
			{
				WorldManager.Instance.onTileMap[xPos, yPos] = biomObject;
			}
		}
	}
	...
	public void placeAnimalDens()
	{
		for (int num = possibleDenPositions.Count - 1; num >= 0; num--)
		{
			...
			for (int i = -5; i < 6; i++)
			{
				for (int j = -5; j < 6; j++)
				{
					if (checkBiomType(Mathf.Clamp(num2 + j, 0, WorldManager.Instance.GetMapSize() - 1), Mathf.Clamp(num3 + i, 0, WorldManager.Instance.GetMapSize() - 1)) == num4 && WorldManager.Instance.onTileMap[Mathf.Clamp(num2 + j, 0, WorldManager.Instance.GetMapSize() - 1), Mathf.Clamp(num3 + i, 0, WorldManager.Instance.GetMapSize() - 1)] == num5)
					{
						flag = true;
						break;
					}
			...
			while (!flag)
			{
				for (int k = -2; k < 2; k++)
				{
					for (int l = -2; l < 2; l++)
					{
						if (checkBiomType(Mathf.Clamp(num2 + l, 0, WorldManager.Instance.GetMapSize() - 1), Mathf.Clamp(num3 + k, 0, WorldManager.Instance.GetMapSize() - 1)) == num4 && mapRand.Range(0, 25) == 2)
						{
							WorldManager.Instance.onTileMap[Mathf.Clamp(num2 + l, 0, WorldManager.Instance.GetMapSize() - 1), Mathf.Clamp(num3 + k, 0, WorldManager.Instance.GetMapSize() - 1)] = num5;
							flag = true;
							break;
						}
					}
	...
	{
		if (mapRand.Range(0f, 100f) < percentChance && WorldManager.Instance.onTileMap[x, y] == -1)
		{
			WorldManager.Instance.onTileMap[x, y] = 30;
		}
	}
	...
	public int getBillabongHeight(int x, int y)
	{
		...
		if (noise > 0.65f)
		{
			if (!((double)noise < 0.7))
			{
				num = (((double)noise < 0.73) ? Mathf.Clamp(WorldManager.Instance.heightMap[x, y] - 1, 1, 4) : (((double)noise < 0.76) ? Mathf.Clamp(WorldManager.Instance.heightMap[x, y] - 2, 1, 4) : ((noise < 0.79f) ? Mathf.Clamp(WorldManager.Instance.heightMap[x, y] - 4, 0, 4) : ((!(noise < 0.81f)) ? Mathf.Clamp(WorldManager.Instance.heightMap[x, y] - 8, -2, 4) : Mathf.Clamp(WorldManager.Instance.heightMap[x, y] - 6, -1, 4)))));
			}
			else
			{
				num = Mathf.Clamp(WorldManager.Instance.heightMap[x, y] - 1, 1, 4);
				if (mapRand.Range(0, 8) < 1 && WorldManager.Instance.tileTypeMap[x, y] != 1)
				{
				}
			}
			if (num <= 0)
			{
				WorldManager.Instance.waterMap[x, y] = true;
			}
			if (num < 1 && num < 0 && biomGrass.getNoise(x + WorldManager.Instance.GetMapSize(), y + WorldManager.Instance.GetMapSize()) > 0.65f && mapRand.Range(0, 5) < 3)
			{
				WorldManager.Instance.onTileMap[x, y] = 121;
			}
		}
		return num;
	}
	...
	public void getRiverGround(float distanceToCenter, float mapHeight, int x, int y, float height, float tropicalCentre, float bushlandCentre)
	{
		if (mapHeight < 0.35f && distanceToCenter > 0.2f)
		{
			...
			WorldManager.Instance.tileTypeMap[x, y] = 3;
			return;
		}
		if (tropicalCentre < 0.65f)
		{
			WorldManager.Instance.tileTypeMap[x, y] = 14;
			if (height >= -1f || height == -4f)
			{
				if (mapRand.Range(0, 18) < 2)
				{
					WorldManager.Instance.onTileMap[x, y] = 130;
				}
				else if (mapRand.Range(0, 10) < 2)
				{
					WorldManager.Instance.onTileMap[x, y] = 131;
				}
				else if (mapRand.Range(0, 8) < 3 && biomGrass.getNoise(x, y) < 0.35f)
				{
					WorldManager.Instance.onTileMap[x, y] = 133;
				}
				else
				{
					WorldManager.Instance.onTileMap[x, y] = -1;
		...
		{
			WorldManager.Instance.tileTypeMap[x, y] = 2;
			if (height == -2f && biomGrass.getNoise(x, y) < 0.35f)
			{
			...
			{
				WorldManager.Instance.onTileMap[x, y] = 2;
			}
			else
			{
				WorldManager.Instance.onTileMap[x, y] = -1;
	...
	{
		int num = WorldManager.Instance.GetMapSize() / 2;
		int i = WorldManager.Instance.GetMapSize() / 2;
		bool flag = false;
		for (; WorldManager.Instance.tileTypeMap[i, num] != 3; i++)
		{
			if (i == WorldManager.Instance.GetMapSize() - 1)
			{
				num++;
				i = 500;
			}
		...
		while (!flag)
		{
			...
			if (WorldManager.Instance.heightMap[i, num] == 1 && WorldManager.Instance.tileTypeMap[i, num] == 3 && WorldManager.Instance.heightMap[i + 1, num] <= 0)
			{
				flag = true;
			}
			else
		...
		WorldManager.Instance.allObjects[215].placeMultiTiledObject(i, num, 2);
		WorldManager.Instance.onTileStatusMap[i, num] = 0;
		TownManager.manage.startingDockPosition = new int[2] { i, num };
		WorldManager.Instance.spawnPos.position = new Vector3(i * 2, 4f, num * 2 + 4);
		if (TownManager.manage.firstConnect)
		{
			tilesToSpawnBuildingsOn.Add(new int[2] { i, num });
			placeAllBuildings();
			WorldManager.Instance.spawnPos.position = TownManager.manage.allShopFloors[13].transform.Find("FirstConnectSpawnPos").position;
		}
	...
	{
		int num = WorldManager.Instance.GetMapSize() / 2 + yDir * 250;
		int num2 = WorldManager.Instance.GetMapSize() / 2 + xDir * 250;
		bool flag = false;
		int num3 = 1;
		while (!flag)
		{
			if (num2 <= 0 || num <= 0 || num >= WorldManager.Instance.GetMapSize() - 1 || num2 >= WorldManager.Instance.GetMapSize() - 1)
			{
				num = WorldManager.Instance.GetMapSize() / 2 + yDir * 250;
				num2 = WorldManager.Instance.GetMapSize() / 2 + xDir * 250;
				if (xDir == 0)
				{
					num2 += num3;
				}
			...
			if (WorldManager.Instance.heightMap[num2, num] >= 1 && WorldManager.Instance.onTileMap[num2, num] >= -1 && WorldManager.Instance.onTileMap[num2, num] != 215)
			{
				flag = true;
				for (int i = 0; i < 2; i++)
				{
					for (int j = 0; j < 2; j++)
					{
						if (WorldManager.Instance.onTileMap[num2 + i, num + j] > -1 && !WorldManager.Instance.allObjectSettings[WorldManager.Instance.onTileMap[num2 + i, num + j]].isWood && !WorldManager.Instance.allObjectSettings[WorldManager.Instance.onTileMap[num2 + i, num + j]].isHardWood && !WorldManager.Instance.allObjectSettings[WorldManager.Instance.onTileMap[num2 + i, num + j]].isSmallPlant && !WorldManager.Instance.allObjectSettings[WorldManager.Instance.onTileMap[num2 + i, num + j]].isStone && !WorldManager.Instance.allObjectSettings[WorldManager.Instance.onTileMap[num2 + i, num + j]].isHardStone)
						{
							flag = false;
							break;
						}
		...
		WorldManager.Instance.allObjects[tileId].placeMultiTiledObject(num2, num, 0);
		WorldManager.Instance.allObjectSettings[tileId].flattenPosUnderMultitiledObject(num2, num, WorldManager.Instance.heightMap[num2, num], 0);
		WorldManager.Instance.onTileStatusMap[num2, num] = 1;
		switch (xDir)
		{
		case -1:
	...
	{
		return Mathf.Sqrt((xHalfPoint - (float)x) * (xHalfPoint - (float)x) + (yHalfPoint - (float)y) * (yHalfPoint - (float)y)) / (float)WorldManager.Instance.GetMapSize();
	}
	...
	public void placeAllBuildings()
	{
		...
		{
			_ = WorldManager.Instance.onTileMap[item[0], item[1]];
			NetworkMapSharer.Instance.requestInterior(item[0], item[1]);
		}
	...
	{
		if (RealWorldTimeLight.time.underGround || !WorldManager.Instance.isPositionOnMap(xPos, yPos))
		{
			return "???";
		}
		int value = checkBiomType(xPos, yPos);
		if (WorldManager.Instance.waterMap[xPos, yPos] && WorldManager.Instance.heightMap[xPos, yPos] < 0 && WorldManager.Instance.waterMap[xPos, yPos] && WorldManager.Instance.tileTypeMap[xPos, yPos] != 3)
		{
			if (WorldManager.Instance.tileTypeMap[xPos, yPos] == 14)
			{
				return "Mangroves";
			}
	...
	public int checkBiomType(int x, int y)
	{
		...
		if (num > 0f)
		{
			...
			{
				if (tropicalClimate.getNoise(x, y) > 0.65f && tropicalClimate.getNoise(x, y) > 0.85f)
				{
					return 17;
				}
			...
			if (bushLandClimate.getNoise(x, y) - num3 > 0f || num3 < 0.35f)
			{
				...
				{
					if (WorldManager.Instance.heightMap[x, y] < 0 && WorldManager.Instance.waterMap[x, y])
					{
						return 2;
					}
					return 3;
		...
		if (WorldManager.Instance.heightMap[x, y] < 0)
		{
			if (tropicalClimate.getNoise(x, y) - num2 > 0f || num2 < 0.35f)
			{
