...
public class TrappedAnimal : NetworkBehaviour
{
	...
	public void RpcOpen()
	{
		...
		SendRPCInternal(typeof(TrappedAnimal), "RpcOpen", writer, 0, includeOwner: true);
	...
	private IEnumerator waitToOpenDelay()
	{
		...
		{
			obs.enabled = false;
			interactBox.SetActive(value: false);
			while (Mathf.Abs(base.transform.position.y - myCarry.dropToPosY) <= 0.15f)
			{
				yield return null;
			}
			yield return new WaitForSeconds(1f);
			yield return StartCoroutine(OpenOnServer());
		}
	}

	public IEnumerator OpenOnServer()
	{
		if (!myCarry.delivered)
		{
			GetComponent<PickUpAndCarry>().RemoveAuthorityBeforeBeforeServerDestroy();
			yield return null;
			RpcOpen();
			if (base.isServer)
			{
				...
				NetworkMapSharer.Instance.spawnAServerDrop(Inventory.Instance.getInvItemId(trapItemDropAfterOpen), 1, base.transform.position);
