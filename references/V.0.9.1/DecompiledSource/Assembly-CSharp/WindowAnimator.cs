...
public class WindowAnimator : MonoBehaviour
{
	...
	private IEnumerator AnimateOpen()
	{
		...
		{
			SoundManager.Instance.soundOnOpenWindow();
		}
		if ((bool)mask)
		{
			...
			while (journey2 <= duration2)
			{
				...
				float num = Mathf.Clamp01(journey2 / duration2);
			...
			{
				journey2 += Time.deltaTime;
				maskChild.color = Color.Lerp(maskChildColor, fadedChildColor, Mathf.Clamp01(journey2 / duration2));
				if (!dontChangeLocalPos)
				{
					myTrans.anchoredPosition = Vector3.Lerp(transformStartingPos - Vector2.up * 10f, transformStartingPos, Mathf.Clamp01(journey2 / duration2));
				}
				yield return null;
			}
			maskChild.enabled = false;
		...
		else
		{
			...
			float duration2 = 0f;
			float journey2 = 0.35f;
			while (duration2 <= journey2)
			{
				duration2 += Time.deltaTime;
				float time = Mathf.Clamp01(duration2 / journey2);
		...
		Inventory.Instance.checkAllClickableButtons();
	}
	...
	private void OnDisable()
	{
		...
		if (playOpenAndCloseSound && (bool)SoundManager.Instance)
		{
			SoundManager.Instance.soundOnCloseWindow();
		}
		Inventory.Instance.checkAllClickableButtons();
	}
