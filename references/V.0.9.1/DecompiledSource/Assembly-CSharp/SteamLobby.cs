...
public class SteamLobby : MonoBehaviour
{
	...
	private void Start()
	{
		...
		m_overlayActivated = Callback<GameOverlayActivated_t>.Create(OnGameOverlay);
	...
	{
		friendSearchButtonText.text = "<sprite=16> " + ConversationGenerator.generate.GetMenuText("Multiplayer_FriendsGamesSearchButton");
		publicSearchButtonText.text = "<sprite=16> " + ConversationGenerator.generate.GetMenuText("Multiplayer_PublicGamesSearchButton");
		if (newType == 0)
		{
	...
	public void searchForLobbies()
	{
		if (SteamManager.Initialized)
		{
			...
			if (searchType == CustomNetworkManager.lobbyType.Friends)
			{
				...
				for (int i = 0; i < friendCount; i++)
				{
					...
					if (SteamFriends.GetFriendGamePlayed(friendByIndex, out var pFriendGameInfo) && pFriendGameInfo.m_steamIDLobby.IsValid() && pFriendGameInfo.m_gameID.AppID() == (AppId_t)1062520u)
					{
		...
		else
		{
			...
			noGamesFound.SetActive(value: false);
		}
	}
	...
	private IEnumerator refreshButtonRoll()
	{
		...
		if (friendButtons.Count <= 0)
		{
			...
			if (friendButtons.Count <= 0)
			{
				noGamesFound.SetActive(value: true);
			}
		}
		else
		{
	...
	private void OnGetLobbyInfo(LobbyDataUpdate_t result)
	{
		for (int i = 0; i < friendButtons.Count; i++)
		{
			if (friendButtons[i].lobbyId.m_SteamID == result.m_ulSteamIDLobby)
			{
				...
				{
					friendButtons[i].gameObject.SetActive(value: false);
				}
				else
				{
					friendButtons[i].gameObject.SetActive(value: true);
				}
			}
		}
	}
	...
	private void OnLobbyEntered(LobbyEnter_t pCallback)
	{
		...
		{
			SteamMatchmaking.SetLobbyData(currentLobby, "characterName", Inventory.Instance.playerName);
			SteamMatchmaking.SetLobbyData(currentLobby, "islandName", Inventory.Instance.islandName);
	...
	public void OnGameOverlay(GameOverlayActivated_t activated)
	{
		InputMaster.input.refreshInputOnSteamOverlay();
	}
}
