...
public class AnimalChunk
{
	...
	public bool needsNewDayRefresh;
	...
	public void clearNonActiveChunk()
	{
		if (!NetworkNavMesh.nav.doesPositionNeedNavChunk(chunkX, chunkY))
		{
			return;
		}
		...
		{
			thisChunk.Add(list[j]);
		}
		needsNewDayRefresh = true;
	}
	...
	public bool isFencedOff(int animalId, int xPos, int yPos)
	{
		...
		{
			if (WorldManager.Instance.fencedOffMap[xPos, yPos] >= 1)
			{
				return true;
			}
			return false;
