...
public class LightTurnsOnAtNight : MonoBehaviour
{
	...
	{
		Invoke("setUpStuff", 1f);
	}

	public void setUpStuff()
	{
		if (!base.isActiveAndEnabled || !RealWorldTimeLight.time)
		{
			return;
		}
	...
	private void Start()
	{
		for (int i = 0; i < lightsToChange.Length; i++)
		{
			lightsToChange[i].renderMode = LightRenderMode.ForceVertex;
		}
	}
	...
	private void turnOnLight()
	{
		...
		else if (RealWorldTimeLight.time.underGround || ((bool)RealWorldTimeLight.time && RealWorldTimeLight.time.currentHour >= RealWorldTimeLight.time.getSunSetTime() + 1))
		{
			turnOnLightsNoDelay();
		}
		else
	...
	private void turnOnLightsNoDelay()
	{
		...
		for (int j = 0; j < lightsToChange.Length; j++)
		{
			...
			if ((flag || RealWorldTimeLight.time.underGround) && base.gameObject.activeInHierarchy)
			{
	...
	{
		WaitForSeconds lightFadeUpdate = new WaitForSeconds(Random.Range(0.85f, 1.1f));
		lightsToChange[lightId].intensity = 0f;
		while (!RealWorldTimeLight.time.underGround && RealWorldTimeLight.time.currentHour == RealWorldTimeLight.time.getSunSetTime())
		{
			yield return lightFadeUpdate;
			float num = Mathf.Lerp(0f, getDesiredInt(lightId), (float)RealWorldTimeLight.time.currentMinute / 60f);
			if (lightsToChange[lightId].intensity != num)
			{
				lightsToChange[lightId].intensity = num;
			}
		}
